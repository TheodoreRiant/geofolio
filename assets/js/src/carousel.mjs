...
function buildCarouselHtml(images, labels) {
    ...
    var slidesHtml = images.map(function(img, i) {
        ...
        return '<div class="mapl-carousel-slide" role="group" aria-roledescription="slide" ' +
    ...
    var dotsHtml = images.map(function(_, i) {
        return '<button type="button" class="mapl-carousel-dot' + (i === 0 ? ' is-active' : '') + '" ' +
               'role="tab" aria-selected="' + (i === 0) + '" data-slide="' + i + '" ' +
               'aria-label="' + escAttr(formatText(l.goToSlide, [i + 1])) + '"></button>';
    }).join('');

    return '<div class="mapl-carousel" role="region" aria-label="' + escAttr(l.gallery) + '" aria-roledescription="' + escAttr(l.carousel) + '" tabindex="0">' +
        '<div class="mapl-carousel-track" data-current="0" style="transform: translateX(0%)">' + slidesHtml + '</div>' +
        '<button type="button" class="mapl-carousel-prev" aria-label="' + escAttr(l.previousPhoto) + '">' +
            '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 18 9 12 15 6"></polyline></svg>' +
        '</button>' +
        '<button type="button" class="mapl-carousel-next" aria-label="' + escAttr(l.nextPhoto) + '">' +
            '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"></polyline></svg>' +
        '</button>' +
        '<div class="mapl-carousel-counter" aria-live="polite">1 / ' + total + '</div>' +
        '<div class="mapl-carousel-dots" role="tablist">' + dotsHtml + '</div>' +
        '</div>';
}
