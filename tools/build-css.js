#!/usr/bin/env node
/**
 * Assemble assets/css/geofolio.css à partir des partiels de assets/css/src/,
 ...
 *
 *   node tools/build-css.js           écrit assets/css/geofolio.css
...
const OUTPUT  = path.join(ROOT, 'assets', 'css', 'geofolio.css');
...
function main() {
    ...
    if (process.argv.includes('--check')) {
        if (fs.readFileSync(OUTPUT, 'utf8') !== css) {
            console.error('assets/css/geofolio.css is out of date: run npm run build:css');
            process.exit(1);
        }
        return;
    }
