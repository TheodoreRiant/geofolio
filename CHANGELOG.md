...
- **Import from a previous map plugin** (`Geofolio\Migration\Legacy`): a migration step described by the `geofolio_legacy_import` filter (old post type, taxonomies, meta, term meta, options, Elementor widget names, shortcodes, type icon catalogue, manager role, slug, labels, appearance, old plugin file). It renames the data in place (IDs and links kept), assigns icons to types without one (exact then partial name match, unmatched types reported), converts the old manager field into people for the imported places only, renames Elementor widgets through the decoded structure (revisions included, written with `update_metadata()` so a revision never overwrites its page) and shortcodes, fills the Labels and Appearance settings only where they are empty, then regenerates the URLs and deactivates the old plugin. The core describes no plugin: a companion provides its predecessor's names. Documented in `docs/hooks.md`.
- Migration steps can require a confirmation (`Geofolio\Migration\ConfirmedStep`): while one is pending, nothing runs automatically. The import shows what it will do, lets the administrator edit the role of the converted people, and runs on confirmation, after the usual snapshot.
